...
def start_script():
    ...
    for vk_domain in config.VK_DOMAIN.split(','):
        logger.info(f"Domain: {vk_domain}")
        last_known_id = read_id(vk_domain)
        ...
        if new_last_id > last_known_id:
            for item in items[::-1]:
                ...
                for item_part in item_parts:
                    ...
                    parsed_post = parse_post(
                        item_parts[item_part], repost_exists, item_part, group_name,vk_domain
                    )
            ...
            write_id(vk_domain, new_last_id)
